...
async function loklokProxyFetch(endpoint, options = {}) {
  const proxyList = [
    '43.109.48.180:9999',
    '8.219.97.248:80',
    '103.43.191.71:8888',
  ...
  const path = endpoint.startsWith('/') ? '/cms/app' + endpoint : endpoint;

  for (const pStr of proxyList) {
    try {
      const [pHost, pPort] = pStr.split(':');
      const resData = await new Promise((resolve, reject) => {
        const req = http.request({
          host: pHost,
          port: parseInt(pPort),
          method: 'CONNECT',
          path: 'ga-mobile-api.loklok.tv:443',
          timeout: 4000
        });
        ...
        req.on('connect', (res, socket) => {
          ...
          const tlsSocket = tls.connect({
            ...
            socket: socket,
            servername: 'ga-mobile-api.loklok.tv'
          }, () => {
            ...
            const headers = {
              ...
              'versioncode': '33',
              'clienttype': 'android_tem3',
              'deviceid': defaultHeaders['deviceid'] || GUEST_DEVICE_ID
            };
            if (postData) {
              headers['Content-Length'] = Buffer.byteLength(postData);
            }

            let reqHeaderStr = (options.method || 'GET') + ' ' + path + ' HTTP/1.1\r\n';
            for (const k in headers) {
              reqHeaderStr += k + ': ' + headers[k] + '\r\n';
            }
            reqHeaderStr += '\r\n';

            tlsSocket.write(reqHeaderStr);
            if (postData) tlsSocket.write(postData);

            let respBuf = '';
            tlsSocket.on('data', chunk => respBuf += chunk.toString('utf8'));
            tlsSocket.on('end', () => {
              try {
                let jsonText = respBuf;
                if (respBuf.includes('{')) {
                  jsonText = respBuf.substring(respBuf.indexOf('{'), respBuf.lastIndexOf('}') + 1);
                }
      ...
      if (resData && (resData.code === '00000' || resData.data)) {
        return resData;
      }
    } catch (_) {}
  }
...
async function loklokFetch(endpoint, options = {}) {
  ...
  try {
    ...
    const isSearchEndpoint = endpoint.includes('searchWithKeyWord') || endpoint.includes('search');
    const directCount = (data && data.data && Array.isArray(data.data.searchResults)) ? data.data.searchResults.length : 0;

    if (isSearchEndpoint && directCount <= 1) {
      try {
        const proxyData = await loklokProxyFetch(endpoint, options);
        if (proxyData && proxyData.data && Array.isArray(proxyData.data.searchResults) && proxyData.data.searchResults.length > directCount) {
          return proxyData;
        }
      } catch (_) {}
    }
